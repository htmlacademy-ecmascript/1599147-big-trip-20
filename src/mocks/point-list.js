export const getPointList = () => (
  [
    {
      'id': '2cc8f108-236b-4aa0-83c0-db5f4b615edc',
      'description': 'Helsinki - middle-eastern paradise',
      'name': 'Helsinki',
      'pictures': [
        {
          'src': 'https://20.objects.pages.academy/static/destinations/16.jpg',
          'description': 'Helsinki for those who value comfort and coziness'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/18.jpg',
          'description': 'Helsinki is a beautiful city'
        }
      ]
    },
    {
      'id': '516da6be-6afd-4599-9083-e155fc4fc0cd',
      'description': 'Saint Petersburg - with crowded streets',
      'name': 'Saint Petersburg',
      'pictures': [
        {
          'src': 'https://20.objects.pages.academy/static/destinations/20.jpg',
          'description': 'Saint Petersburg is a beautiful city'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/18.jpg',
          'description': 'Saint Petersburg middle-eastern paradise'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/1.jpg',
          'description': 'Saint Petersburg in a middle of Europe'
        }
      ]
    },
    {
      'id': '6e0aca62-3eec-4d6b-9085-5077abf4e57d',
      'description': 'Kopenhagen - with a beautiful old town',
      'name': 'Kopenhagen',
      'pictures': [
        {
          'src': 'https://20.objects.pages.academy/static/destinations/9.jpg',
          'description': 'Kopenhagen with crowded streets'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/1.jpg',
          'description': 'Kopenhagen with an embankment of a mighty river as a centre of attraction'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/8.jpg',
          'description': 'Kopenhagen a perfect place to stay with a family'
        }
      ]
    },
    {
      'id': '6c3ea12b-5389-4f13-9d74-43988ce75ce3',
      'description': 'Milan - with a beautiful old town',
      'name': 'Milan',
      'pictures': [
        {
          'src': 'https://20.objects.pages.academy/static/destinations/19.jpg',
          'description': 'Milan middle-eastern paradise'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/8.jpg',
          'description': 'Milan for those who value comfort and coziness'
        }
      ]
    },
    {
      'id': '7863ed76-e512-4b0f-893a-90136a530ef0',
      'description': 'Monaco - middle-eastern paradise',
      'name': 'Monaco',
      'pictures': [
        {
          'src': 'https://20.objects.pages.academy/static/destinations/1.jpg',
          'description': 'Monaco for those who value comfort and coziness'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/1.jpg',
          'description': 'Monaco full of of cozy canteens where you can try the best coffee in the Middle East'
        },
        {
          'src': 'https://20.objects.pages.academy/static/destinations/2.jpg',
          'description': 'Monaco middle-eastern paradise'
        }
      ]
    }
  ]
);
